...
export default function SectionItem({
  ...
  // Initialize contents from section data
  useEffect(() => {
    const lectureContents: Lecture[] = (section.lectures || []).map(lecture => ({
      ...lecture,
    ...
    setContents([...lectureContents, ...quizContents].sort((a, b) => a.position - b.position))
  ...
  const saveSectionMutation = useMutation({
    mutationFn: async (values: { title: string; description: string }) => {
      ...
      if (isNewSection) {
        ...
        return await Axios.put(`${endpoint}/${section.id}`, {
          title: values.title,
          description: values.description,
          position: section.position
        })
  ...
  const generateId = () => `temp-content-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`
        ...
        <div className="border border-gray-200 rounded-lg bg-white">
          ...
          <CollapsibleContent>
            <div className="px-4 pb-4 space-y-4 border-t border-gray-100 pt-4">
              ...
              {/* Content Section (Lectures & Quizzes) */}
                ...
                <div className="space-y-2">
                  ...
                  {contents.length === 0 && (
                    <p className="text-sm text-gray-400 text-center py-4">
                      No content yet. Click "Add Lecture" or "Add Quiz" to create one.
                    </p>
                  )}
                </div>
              </div>
