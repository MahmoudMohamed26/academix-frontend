    ...
    >
      ...
      >
        <div
          ...
          className="bg-sidebar group-data-[variant=floating]:border-sidebar-border justify-between flex h-full w-full flex-col group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border group-data-[variant=floating]:shadow-sm"
        >
          {children}
        </div>
      </div>
...
function SidebarTrigger({
  ...
  return (
    <button
      ...
      variant="ghost"
      size="icon"
      className={cn("size-12 text-(--main-color) hover:text-(--main-darker-color) duration-300 w-fit h-fit p-1 rounded-sm cursor-pointer", className)}
      onClick={(event) => {
        onClick?.(event)
        toggleSidebar()
      }}
